        ...
        if ($secureHash == $vnp_SecureHash) {
            ...
            if ($_GET['vnp_ResponseCode'] == '00') {
                ...
                try {
                    ...
                    $stmt_user = $conn->prepare("SELECT user_id FROM orders WHERE id = ? LIMIT 1");
                    $stmt_user->bind_param("i", $order_id);
                    $stmt_user->execute();
                    $order_user = $stmt_user->get_result()->fetch_assoc();

                    if (!empty($order_user['user_id'])) {
                        $user_id = (int) $order_user['user_id'];
                        $stmt_items = $conn->prepare("SELECT banh_id, quantity FROM order_items WHERE order_id = ?");
                        $stmt_items->bind_param("i", $order_id);
                        $stmt_items->execute();
                        $items = $stmt_items->get_result()->fetch_all(MYSQLI_ASSOC);

                        $stmt_update_cart = $conn->prepare(
                            "UPDATE cart SET quantity = quantity - ? WHERE user_id = ? AND banh_id = ?"
                        );
                        $stmt_cleanup_cart = $conn->prepare(
                            "DELETE FROM cart WHERE user_id = ? AND banh_id = ? AND quantity <= 0"
                        );

                        foreach ($items as $item) {
                            $qty = (int) $item['quantity'];
                            $banh_id = (int) $item['banh_id'];
                            $stmt_update_cart->bind_param("iii", $qty, $user_id, $banh_id);
